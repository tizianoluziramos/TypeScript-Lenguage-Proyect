export class ГенераторКлючейWindows95 {
    // Считает количество цифр в числе
    static количествоЦифр(num: number): number {
        let ct = 0;
        while (num > 0) {
            ct++;
    ...
    static суммаЦифр(num: number): number {
        let sm = 0;
        while (num > 0) {
            const rem = num % 10;
            sm += rem;
    ...
    // Генератор OEM ключа
    // Формат: ABCYY-OEM-0XXXXXX-XXXXX
    static ключOEM(): string {
