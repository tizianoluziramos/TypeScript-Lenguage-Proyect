export class GerarChavesDeWindows95 {
    // Conta o número de dígitos de um número
    static numDigitos(num: number): number {
        let ct = 0;
        while (num > 0) {
            ct++;
    ...
    static somaDosDigitos(num: number): number {
        let sm = 0;
        while (num > 0) {
            const rem = num % 10;
            sm += rem;
    ...
    static chaveCD(): string {
        ...
        let x2 = 1;
        while (this.somaDosDigitos(x2) % 7 !== 0) {
            x2 = Math.floor(Math.random() * 10000000);
            while (x2 % 10 === 0 || x2 % 10 === 8 || x2 % 10 === 9) {
                x2 = Math.floor(Math.random() * 10000000);
            }
        }
        const length = this.numDigitos(x2);
        const x2str = x2.toString().padStart(7, '0');
        
        return `${x1str}-${x2str}`;
    }

    // Gerador de chave OEM
    // Formato: ABCYY-OEM-0XXXXXX-XXXXX
    static chaveOEM(): string {
